#!/bin/bash
function checkOldSpaceXStorage()
{
    verbose INFO "Checking old spacex storage..." h
    local ret=0
    if [ ! -e "$spacexdir/spacex-storage" ]; then
...
realsworkerdir=$spacestoragedir/$newversion
...
# check previous spacex-storage
checkOldSpaceXStorage
...
if [ "$DOCKERMODLE" == "0" ]; then
    ...
    res=0
    chown -R $uid:$uid $spacestoragedir
    res=$(($?|$res))
    chown -R $uid:$uid $spacextooldir
    res=$(($?|$res))
    checkRes $res "quit" "success"
fi

verbose INFO "Spacex-storage has been installed in /opt/spacex/spacex-storage/$newversion!"
instSuccess=true
