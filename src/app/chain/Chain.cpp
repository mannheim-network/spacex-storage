...
/**
 * @description: waiting for the spacex chain to run
...
size_t Chain::get_offline_block_height(void)
{
    ...
    if (spacex::DataBase::get_instance()->get("offline_block_height_key", offline_base_height_str) == SPACEX_SUCCESS)
    {
...
{
    offline_block_height_mutex.lock();
    spacex::DataBase::get_instance()->set("offline_block_height_key", std::to_string(this->offline_block_height + h));
