...
std::string config_file_path = SPACEX_INST_DIR "/etc/Config.json";
...
bool Config::init(std::string path)
{
    ...
    // Fill configurations
    json::JSON config_value = json::JSON::Load(&spacex_status, config_str);
    ...
    if (SPACEX_SUCCESS != (spacex_status = create_directory(this->db_path)))
    {
        p_log->err("Create path:'%s' failed! Error code:%lx\n", this->db_path.c_str(), spacex_status);
        return false;
    }
    ...
    for (int i = 0; i < data_paths.size(); i++)
    {
        ...
        if (SPACEX_SUCCESS != (spacex_status = create_directory(d_path)))
        {
            p_log->err("Create path:'%s' failed! Error code:%lx\n", d_path.c_str(), spacex_status);
            return false;
        }
    }
...
bool Config::config_file_add_data_paths(const json::JSON &paths)
{
    ...
    else
    {
        uint8_t *p_data = NULL;
        size_t data_size = 0;
        if (SPACEX_SUCCESS != (spacex_status = get_file(config_file_path.c_str(), &p_data, &data_size)))
        {
        ...
        {
            json::JSON config_json = json::JSON::Load(&spacex_status, p_data, data_size);
            if (SPACEX_SUCCESS != spacex_status)
            {
            ...
            replace(config_str, "\\\\", "\\");
            spacex_status = save_file(config_file_path.c_str(), reinterpret_cast<const uint8_t *>(config_str.c_str()), config_str.size());
            if (SPACEX_SUCCESS != spacex_status)
            {
