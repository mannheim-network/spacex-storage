#ifndef _SPACEX_API_HANDLER_H_
#define _SPACEX_API_HANDLER_H_
...
#define WS_SEPARATOR "$spacex_ws_separator$"
    ...
    http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send, bool /*is_ssl*/)
{
    ...
    if(req.method() == http::verb::get)
    {
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else if (SGX_SUCCESS != (sgx_status = Ecall_enable_upgrade(global_eid, &spacex_status, block_header.number)))
            {
            ...
            else
            {
                ...
                else
                {
                    if (UPGRADE_STATUS_COMPLETE == ed->get_upgrade_status())
                    {
                        ...
                        spacex::DataBase *db = spacex::DataBase::get_instance();
                        std::string metadata_old;
                        if (SPACEX_SUCCESS != (spacex_status = db->get(ID_METADATA, metadata_old)))
                        {
                            ret_info = "Upgrade: get old metadata failed!Status code:" + num_to_hexstring(spacex_status);
                            p_log->warn("%s\n", ret_info.c_str());
                        }
                        else
                        {
                            if (SPACEX_SUCCESS != (spacex_status = db->set(ID_METADATA_OLD, metadata_old)))
                            {
                                ret_info = "Upgrade: store old metadata failed!Status code:" + num_to_hexstring(spacex_status);
                                p_log->warn("%s\n", ret_info.c_str());
                            }
                        }
                        if (SPACEX_SUCCESS != (spacex_status = db->del(ID_METADATA)))
                        {
                            ret_info = "Upgrade: delete old metadata failed!Status code:" + num_to_hexstring(spacex_status);
                            p_log->warn("%s\n", ret_info.c_str());
                        }
                        else
                        {
                ...
                ret_body[HTTP_STATUS_CODE] = std::stoi(num_to_hexstring(spacex_status), NULL, 10);
                ret_body[HTTP_MESSAGE] = ret_info;
                res.body() = ret_body.dump();
            }
    ...
    if(req.method() == http::verb::post)
    {
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else
            {
                ...
                ret_code = 200;
                // Store debug flag
                spacex_status_t ret = spacex::DataBase::get_instance()->set(DB_DEBUG, std::to_string(debug_flag));
                if (SPACEX_SUCCESS != ret)
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (change_srd_num > 0)
                    {
                        ...
                        if (true_increase <= 0)
                        {
                            ret_info = "No more srd can be added. Use 'sudo spacex tools workload' to check.";
                            ret_code = 400;
                            goto change_end;
                        }
                    ...
                    else
                    {
                        ...
                        if (true_decrease <= 0)
                        {
                            ret_info = "No srd space to be deleted. Use 'sudo spacex tools workload' to check.";
                            ret_code = 400;
                            goto change_end;
                        }
                    ...
                    // Start changing srd
                    long real_change = 0;
                    if (SGX_SUCCESS != Ecall_change_srd_task(global_eid, &spacex_status, change_srd_num, &real_change))
                    {
                    ...
                    else
                    {
                        ...
                        case SPACEX_SRD_NUMBER_EXCEED:
                            ...
                            ret_code = 200;
                            break;
                        case SPACEX_UPGRADE_IS_UPGRADING:
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else
            {
                ...
                {
                    ret_info = "Unexpected error: " + num_to_hexstring(spacex_status);
                    ret_code = 500;
                }  
            }
            ret_body[HTTP_STATUS_CODE] = ret_code;
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            else
            {
                ...
                // Do start seal
                if (SGX_SUCCESS != (sgx_status = Ecall_seal_file_start(global_eid, &spacex_status, cid.c_str(), cid_b58.c_str())))
                {
                ...
                else if (SPACEX_SUCCESS != spacex_status)
                {
                    switch (spacex_status)
                    {
                        case SPACEX_FILE_NUMBER_EXCEED:
                        ...
                        case SPACEX_UPGRADE_IS_UPGRADING:
                            ...
                            ret_code = 503;
                            break;
                        case SPACEX_STORAGE_FILE_DUP:
                            ret_info = "This file '" + cid + "' has been sealed";
                            p_log->info("%s\n", ret_info.c_str());
                            ret_code = 200;
                            break;
                        case SPACEX_STORAGE_FILE_SEALING:
                            ret_info = "Same file '" + cid + "' is being sealed.";
                            p_log->info("%s\n", ret_info.c_str());
                            ret_code = 200;
                            break;
                        case SPACEX_STORAGE_FILE_DELETING:
                            ret_info = "Same file '" + cid + "' is being deleted.";
                            p_log->info("%s\n", ret_info.c_str());
                            ret_code = 400;
                            break;
                        default:
                            ret_info = "Seal file '" + cid + "' failed! Unexpected error, error code:" + num_to_hexstring(spacex_status);
                            p_log->err("%s\n", ret_info.c_str());
                            ret_code = 500;
                    }
                }
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            if (SGX_SUCCESS != (sgx_status = Ecall_seal_file(global_eid, &spacex_status, cid.c_str(), sealed_data, sealed_data_sz, is_link, index_path, IPFS_INDEX_LENGTH)))
            {
            ...
            else if (SPACEX_SUCCESS != spacex_status)
            {
                switch (spacex_status)
                {
                    ...
                    case SPACEX_UPGRADE_IS_UPGRADING:
                        ...
                        ret_code = 503;
                        break;
                    case SPACEX_STORAGE_NEW_FILE_NOTFOUND:
                        ret_info = "Seal file '" + cid + "' failed, no request or file has been removed";
                        p_log->debug("%s\n", ret_info.c_str());
                        ret_code = 500;
                        break;
                    case SPACEX_STORAGE_NO_ENOUGH_SPACE:
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            {
                std::string cid = req_json[param_name].ToString();
                if (SGX_SUCCESS != (sgx_status = Ecall_seal_file_end(global_eid, &spacex_status, cid.c_str())))
                {
                    ret_info = "Start seal file '%s' failed! Invoke SGX API failed! Error code:" + num_to_hexstring(sgx_status);
                    p_log->err("%s\n", ret_info.c_str());
                    ret_code = 500;
                ...
                else if (SPACEX_SUCCESS != spacex_status)
                {
                    switch (spacex_status)
                    {
                        ...
                        case SPACEX_STORAGE_NEW_FILE_NOTFOUND:
                            ...
                            ret_code = 404;
                            break;
                        case SPACEX_STORAGE_INCOMPLETE_BLOCK:
                            ret_info = "Seal file '" + cid + "' failed due to incomplete file blocks";
                            p_log->info("%s\n", ret_info.c_str());
                            ret_code = 500;
                            break;
                        default:
                            ret_info = "Seal file '" + cid + "' failed! Error code:" + num_to_hexstring(spacex_status);
                            p_log->info("%s\n", ret_info.c_str());
                            ret_code = 500;
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (SGX_SUCCESS != (sgx_status = Ecall_unseal_file(global_eid, &spacex_status, index_path.c_str(), p_decrypted_data, decrypted_data_sz, &decrypted_data_sz_r)))
                        {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                switch (spacex_status)
                                {
                                case SPACEX_UNSEAL_DATA_FAILED:
                                ...
                                case SPACEX_UPGRADE_IS_UPGRADING:
                                    ...
                                    break;
                                default:
                                    ret_info = "Unseal data failed! Error code:" + num_to_hexstring(spacex_status);
                                    p_log->err("%s\n", ret_info.c_str());
                                    ret_code = 404;
                                }
                            }
        ...
        if (req_route.size() == cur_path.size() && req_route.compare(cur_path) == 0)
        {
            ...
            sgx_status_t sgx_status = SGX_SUCCESS;
            spacex_status_t spacex_status = SPACEX_SUCCESS;
            json::JSON req_json = json::JSON::Load(&spacex_status, req.body().data(), req.body().size());
            if (SPACEX_SUCCESS != spacex_status)
            {
            ...
            else
            {
                ...
                {
                    if (SGX_SUCCESS != (sgx_status = Ecall_recover_illegal_file(global_eid, &spacex_status, req.body().data(), req.body().size())))
                    {
