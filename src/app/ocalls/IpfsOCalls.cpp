...
spacex_status_t ocall_ipfs_get_block(const char *cid, uint8_t **p_data, size_t *data_size)
{
    *data_size = Ipfs::get_instance()->block_get(cid, p_data);
    if (*data_size == 0)
    {
        ...
        return SPACEX_STORAGE_IPFS_BLOCK_GET_ERROR;
    }
...
spacex_status_t ocall_save_ipfs_block(const char *path, const uint8_t *data, size_t data_size, char *uuid, size_t /*uuid_len*/)
{
    ...
    for (size_t i = 0; i < choose_len/FILE_DISK_LIMIT;)
    {
        ...
        if (reserved > data_size * 4)
        {
            ...
            if (uuid_str.size() == UUID_LENGTH * 2)
            {
                ...
                if (SPACEX_SUCCESS == save_file_ex(file_path.c_str(), data, data_size, mode_t(0664), SF_CREATE_DIR))
                {
    ...
    return SPACEX_STORAGE_NO_ENOUGH_SPACE;
}
...
spacex_status_t ocall_ipfs_cat(const char *cid, uint8_t **p_data, size_t *data_size)
{
    ...
    *data_size = Ipfs::get_instance()->cat(cid, p_data);
    if (*data_size == 0)
    {
        return SPACEX_STORAGE_IPFS_CAT_ERROR;
    }
    return SPACEX_SUCCESS;
}
...
spacex_status_t ocall_ipfs_add(uint8_t *p_data, size_t len, char *cid, size_t /*cid_len*/)
{
    ...
    if (cid_str.size() == 0)
    {
        return SPACEX_STORAGE_IPFS_ADD_ERROR;
    }
...
spacex_status_t ocall_ipfs_del_all(const char *cid)
{
    ...
    // Delete sealed tree
    spacex::DataBase::get_instance()->del(cid);
