...
spacex_status_t entry_network()
{
    ...
    do
    {
        ...
        switch (status)
        {
            ...
            case SGX_ERROR_UPDATE_NEEDED:
                ...
                return SPACEX_DEVICE_ERROR;
            default:
                p_log->err("SGX init quote failed!Error code: %lx\n", status);
                return SPACEX_INIT_QUOTE_FAILED;
        }
    ...
    sgx_status_t status_ret = Ecall_verify_and_upload_identity(global_eid, &spacex_status, const_cast<char**>(ias_report.data()), ias_report.size());
