...
bool initialize_enclave()
{
    ...
    if (SPACEX_SUCCESS == get_file(SGX_WL_FILE_PATH, &p_wl_data, &wl_data_size))
    {
...
{
    // Create path
    if (SPACEX_SUCCESS != create_directory(p_config->base_path))
    {
    ...
    // Init spacex
    if (spacex::Chain::get_instance() == NULL)
    {
        p_log->err("Init spacex chain failed!\n");
        return false;
    }

    // Initialize DataBase
    if (spacex::DataBase::get_instance() == NULL)
    {
    ...
    // Restore debug flag
    spacex::Log::get_instance()->restore_debug_flag();
...
restore_try_again:
    ...
    sgx_status = safe_ecall_store2(global_eid, &spacex_status, ECALL_RESTORE_FROM_UPGRADE, reinterpret_cast<const uint8_t *>(p_meta), meta_size);
    ...
    {
        if (SPACEX_INIT_QUOTE_FAILED == spacex_status)
        {
            if (--restore_tryout > 0)
            {
...
entry_network_flag:
    ...
    while (true)
    {
        ...
        if (UPGRADE_STATUS_END == ed->get_upgrade_status())
        {
            ...
            {
                p_log->err("Get block header failed! Please check your spacex-api and spacex chain!\n");
            }
            else if (SGX_SUCCESS != (sgx_status = Ecall_gen_upgrade_data(global_eid, &spacex_status, block_header.number)))
            {
