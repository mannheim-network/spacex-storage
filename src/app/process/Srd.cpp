...
bool check_or_init_disk(std::string path)
{
    ...
    if (access(uuid_file.c_str(), R_OK) != -1)
    {
        ...
        if (!ed->is_disk_exist(path))
        {
            uint8_t *p_data = NULL;
            size_t data_sz = 0;
            if (SPACEX_SUCCESS != (spacex_status = get_file(uuid_file.c_str(), &p_data, &data_sz)))
            {
                p_log->err("Get existed path:%s uuid failed! Error code:%lx\n", uuid_file.c_str(), spacex_status);
                return false;
            }
    ...
    else
    {
        if (!ed->is_disk_exist(path))
        {
            ...
            spacex_status = save_file_ex(uuid_file.c_str(), reinterpret_cast<const uint8_t *>(uuid.c_str()), uuid.size(), 0444, SF_CREATE_DIR);
            if (SPACEX_SUCCESS != spacex_status)
            {
                p_log->err("Save uuid file to path:'%s' failed! Error code:%lx\n", path.c_str(), spacex_status);
                return false;
            }
    ...
    if (SPACEX_SUCCESS != create_directory(srd_dir))
    {
 ...
 */
spacex_status_t srd_change(long change)
{
    Config *p_config = Config::get_instance();
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    ...
    if (change > 0)
    {
        ...
        if (true_increase <= 0)
        {
            ...
            return SPACEX_SRD_NUMBER_EXCEED;
        }
        ...
        std::vector<std::shared_ptr<std::future<spacex_status_t>>> tasks_v;
        ...
        while (task > 0 && !srd_end)
        {
            ...
            for (int i = 0; i < inc_srd_json.size(); i++)
            {
                if (inc_srd_json[i][WL_DISK_USE].ToInt() > 0)
                {
                    ...
                    tasks_v.push_back(std::make_shared<std::future<spacex_status_t>>(pool.push([eid, uuid](int /*id*/){
                        spacex_status_t inc_spacex_ret = SPACEX_SUCCESS;
                        sgx_status_t inc_sgx_ret = Ecall_srd_increase(eid, &inc_spacex_ret, uuid.c_str());
                        if (SGX_SUCCESS != inc_sgx_ret)
                        {
                            ...
                            if (SPACEX_SUCCESS == inc_spacex_ret)
                            {
                                inc_spacex_ret = SPACEX_SGX_FAILED;
                            }
                        }
                        decrease_running_srd_task();
                        return inc_spacex_ret;
                    })));
                }
            }
        }
        ...
        if (srd_success_num < (size_t)true_increase)
        {
            ...
            spacex_status_t change_spacex_ret = SPACEX_SUCCESS;
            long real_change = 0;
            sgx_status_t change_sgx_ret = Ecall_change_srd_task(global_eid, &change_spacex_ret, left, &real_change);
