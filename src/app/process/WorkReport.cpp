...
void work_report_loop(void)
{
    ...
    spacex::Chain *p_chain = spacex::Chain::get_instance();
    ...
    while (true)
    {
        ...
        if (SGX_SUCCESS != (sgx_status = Ecall_gen_and_upload_work_report(global_eid, &spacex_status,
        ...
        else if (SPACEX_SUCCESS != spacex_status)
        {
            ...
            case SPACEX_UPGRADE_IS_UPGRADING:
                ...
                break;
            case SPACEX_SGX_SIGN_FAILED:
                p_log->err("Can't generate work report, SGX signed failed.");
                break;
            case SPACEX_WORK_REPORT_NOT_VALIDATED:
