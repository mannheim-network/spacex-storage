...
spacex_status_t rename_dir(const std::string &old_path, const std::string &new_path)
{
    ...
    {
        return SPACEX_RENAME_FILE_FAILED;
    }
...
spacex_status_t save_file_ex(const char *path, const uint8_t *data, size_t data_size, mode_t mode, save_file_type_t type)
{
    if (SF_CREATE_DIR == type)
    {
        ...
        if (last_slash != 0 && last_slash != std::string::npos)
        {
            ...
            if (SPACEX_SUCCESS != (spacex_status = create_directory(dir_path)))
            {
                return spacex_status;
            }
        }
    ...
    {
        spacex_status = SPACEX_WRITE_FILE_FAILED;
