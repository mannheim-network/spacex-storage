...
void ecall_main_loop()
{
    ...
    while (true)
    {
        ...
        if (SPACEX_SUCCESS != (spacex_status = id_store_metadata()))
        {
 ...
 */
spacex_status_t ecall_srd_increase(const char *uuid)
{
 ...
 */
spacex_status_t ecall_change_srd_task(long change, long *real_change)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
 ...
 */
spacex_status_t ecall_restore_metadata()
{
 ...
 */
spacex_status_t ecall_cmp_chain_account_id(const char *account_id, size_t len)
{
 ...
 * @return: Sign status
 */
spacex_status_t ecall_gen_and_upload_work_report(const char *block_hash, size_t block_height)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
    ...
    spacex_status_t ret = gen_and_upload_work_report(block_hash, block_height, 0, false, true);
 ...
 */
spacex_status_t ecall_verify_and_upload_identity(char **IASReport, size_t len)
{
 ...
 */
spacex_status_t ecall_seal_file_start(const char *root, const char *root_b58)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
    ...
    spacex_status_t ret = storage_seal_file_start(root, root_b58);
 ...
 * @return: Seal status
 */
spacex_status_t ecall_seal_file(const char *root,
                               const uint8_t *data,
                               size_t data_size,
                               bool is_link,
                               char *path,
 ...
 * @return: Inform result
 */
spacex_status_t ecall_seal_file_end(const char *root)
{
 ...
 */
spacex_status_t ecall_unseal_file(const char *path, uint8_t *p_decrypted_data, size_t decrypted_data_size, size_t *p_decrypted_data_size)
{
    spacex_status_t ret = storage_unseal_file(path, p_decrypted_data, decrypted_data_size, p_decrypted_data_size);
 ...
 */
spacex_status_t ecall_delete_file(const char *cid)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
    ...
    spacex_status_t ret = storage_delete_file(cid);
 ...
 */
spacex_status_t ecall_enable_upgrade(size_t block_height)
{
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    Workload *wl = Workload::get_instance();
    if (SPACEX_SUCCESS == (spacex_status = wl->can_report_work(block_height)))
    {
 ...
 */
spacex_status_t ecall_gen_upgrade_data(size_t block_height)
{
 ...
 */
spacex_status_t ecall_restore_from_upgrade(const uint8_t *data, size_t data_size)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
 ...
 */
spacex_status_t ecall_recover_illegal_file(const uint8_t *data, size_t data_size)
{
 ...
 * @return: Store result
 */
spacex_status_t ecall_safe_store2(ecall_store_type_t t,
                                 const uint8_t *data,
                                 size_t total_size,
                                 size_t partial_size,
                                 size_t offset,
                                 uint32_t buffer_key)
{
    ...
    spacex_status = (g_ecall_store2_func_m[t])(g_ecall_buffer_pool[buffer_key], total_size);
