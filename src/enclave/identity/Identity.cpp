...
spacex_status_t id_verify_and_upload_identity(char **IASReport, size_t size)
{
    ...
    vector<X509 *> certvec;
    size_t sigsz;
    spacex_status_t status = SPACEX_SUCCESS;
    size_t qbsz;
    ...
    if (certchain == "")
    {
        return SPACEX_IAS_BAD_CERTIFICATE;
    }
    ...
    if (certar == NULL)
    {
        return SPACEX_IAS_INTERNAL_ERROR;
    }
    ...
    if (ias_sig == "")
    {
        return SPACEX_IAS_BAD_SIGNATURE;
    }
    ...
    if (pkey == NULL)
    {
        return SPACEX_IAS_GETPUBKEY_FAILED;
    }
    ...
    {
        return SPACEX_IAS_REPORTDATA_NE;
    }
    ...
    {
        return SPACEX_IAS_BADMEASUREMENT;
    }
...
spacex_status_t id_store_metadata()
{
    ...
    do
    {
        ...
        meta_data = meta_json.dump_vector(&spacex_status);
        if (SPACEX_SUCCESS != spacex_status)
        {
            return spacex_status;
        }
    } while (0);
    if (SPACEX_SUCCESS != (spacex_status = vector_end_insert(meta_buffer, meta_data)))
    {
        return spacex_status;
    }

    spacex_status = persist_set(ID_METADATA, meta_buffer.data(), meta_buffer.size());
...
spacex_status_t id_restore_metadata()
{
    ...
    uint8_t *p_data = NULL;
    size_t data_len = 0;
    spacex_status_t spacex_status = persist_get(ID_METADATA, &p_data, &data_len);
    if (SPACEX_SUCCESS != spacex_status)
    {
    ...
    if (meta_json.size() == 0)
    {
        ...
        return SPACEX_INVALID_META_DATA;
    }
    ...
    // ----- Restore metadata ----- //
    Defer def_clean_all([&spacex_status, &wl](void) { 
        if (SPACEX_SUCCESS != spacex_status)
        {
            wl->clean_all(); 
        }
    ...
    // Restore srd
    if (SPACEX_SUCCESS != (spacex_status = wl->restore_srd(meta_json[ID_SRD])))
    {
        return spacex_status;
    }
    log_debug("Restore srd successfully!\n");
    // Restore file
    if (SPACEX_SUCCESS != (spacex_status = wl->restore_file(meta_json[ID_FILE])))
    {
        return spacex_status;
    }
    ...
    if (SPACEX_SUCCESS != (spacex_status = wl->restore_pre_pub_key(meta_json)))
    {
        return spacex_status;
    }
...
spacex_status_t id_gen_upgrade_data(size_t block_height)
{
    ...
    {
        return SPACEX_BLOCK_HEIGHT_EXPIRED;
    }
    if (block_height < REPORT_SLOT + wl->get_report_height())
    {
        return SPACEX_UPGRADE_WAIT_FOR_NEXT_ERA;
    }
    ...
    ocall_get_block_hash(&spacex_status, report_height, report_hash, HASH_LENGTH * 2);
    if (SPACEX_SUCCESS != spacex_status)
    {
    ...
    if (SPACEX_SUCCESS != (spacex_status = gen_and_upload_work_report(report_hash, report_height, random_time, false, false)))
    {
        log_err("Fatal error! Send work report failed! Error code:%lx\n", spacex_status);
        return SPACEX_UPGRADE_GEN_WORKREPORT_FAILED;
    }
    ...
    json::JSON srd_json = wl->get_upgrade_srd_info(&spacex_status);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
    log_debug("Serialize srd data successfully!\n");
    json::JSON file_json = wl->get_upgrade_file_info(&spacex_status);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
    ...
    do
    {
        ...
        upgrade_data = upgrade_json.dump_vector(&spacex_status);
        if (SPACEX_SUCCESS != spacex_status)
        {
            return spacex_status;
        }
    ...
    // Store upgrade data
    spacex_status = safe_ocall_store2(OCALL_STORE_UPGRADE_DATA, upgrade_data.data(), upgrade_data.size());
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
...
spacex_status_t id_restore_from_upgrade(const uint8_t *data, size_t data_size)
{
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    sgx_status_t sgx_status = SGX_SUCCESS;
    json::JSON upgrade_json = json::JSON::Load(&spacex_status, data, data_size);
    if (SPACEX_SUCCESS != spacex_status)
    {
    ...
    // ----- Restore workload ----- //
    // Restore srd
    if (SPACEX_SUCCESS != (spacex_status = wl->restore_srd(upgrade_json[UPGRADE_SRD])))
    {
        log_err("Restore srd failed! Error code:%lx\n", spacex_status);
        return SPACEX_UPGRADE_RESTORE_SRD_FAILED;
    }
    log_debug("Restore srd data successfully!\n");
    // Restore file
    if (SPACEX_SUCCESS != (spacex_status = wl->restore_file(upgrade_json[UPGRADE_FILE])))
    {
        log_err("Restore file failed! Error code:%lx\n", spacex_status);
        return SPACEX_UPGRADE_RESTORE_FILE_FAILED;
    }
    log_debug("Restore file data successfully!\n");

    // ----- Verify workload signature ----- //
    json::JSON wl_info = wl->gen_workload_info(&spacex_status);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
    ...
    if (SGX_SUCCESS != sgx_status || p_result != SGX_EC_VALID)
    {
        ...
        return SPACEX_SGX_VERIFY_SIG_FAILED;
    }
    ...
    if (wl_info[WL_FILE_ROOT_HASH].ToString().compare(upgrade_files_root_str) != 0)
    {
        ...
        return SPACEX_UPGRADE_BAD_FILE;
    }

    // ----- Entry network ----- //
    wl->set_upgrade(sgx_a_pub_key);
    ocall_entry_network(&spacex_status);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
    ...
    if (SPACEX_SUCCESS != (spacex_status = gen_and_upload_work_report(report_hash_str.c_str(), std::atoi(report_height_str.c_str()), 0, true, true)))
    {
        return spacex_status;
    }
