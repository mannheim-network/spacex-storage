 ...
 * @return: sign status
 */
spacex_status_t gen_and_upload_work_report(const char *block_hash, size_t block_height, long wait_time, bool is_upgrading, bool locked)
{
 ...
 * @return: sign status
 */
spacex_status_t gen_work_report(const char *block_hash, size_t block_height, bool is_upgrading)
{
    ...
    do
    {
        ...
        wr_data = wr_json.dump_vector(&spacex_status);
        if (SPACEX_SUCCESS != spacex_status)
        {
            return spacex_status;
        }
