 ...
 * @return: Save status
 */
spacex_status_t save_file(const char *g_path, size_t index, sgx_sha256_hash_t hash, const unsigned char *data, size_t data_size)
{
    std::string file_path = get_leaf_path(g_path, index, hash);
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    ocall_save_file(&spacex_status, file_path.c_str(), data, data_size, STORE_TYPE_SRD);
 ...
 * @return: Save status
 */
spacex_status_t save_m_hashs_file(const char *g_path, const unsigned char *data, size_t data_size)
{
...
void srd_change()
{
    ...
    if (srd_change_num != 0)
    {
        ocall_srd_change(&spacex_status, srd_change_num);
        if (SPACEX_SRD_NUMBER_EXCEED == spacex_status)
        {
            sl_task.lock();
            g_srd_task = 0;
            sl_task.unlock();
    ...
    if (SPACEX_SUCCESS != persist_set_unsafe(WL_SRD_REMAINING_TASK, reinterpret_cast<const uint8_t *>(srd_task_str.c_str()), srd_task_str.size()))
    {
...
spacex_status_t srd_increase(const char *uuid)
{
    ...
    if (!wl->report_has_validated_proof())
    {
        return SPACEX_VALIDATE_HIGH_PRIORITY;
    }
    ...
    // ----- Generate srd file ----- //
    // Create directory
    ocall_create_dir(&spacex_status, tmp_dir.c_str(), STORE_TYPE_SRD);
    ...
    {
        log_err("Malloc memory failed!\n");
        return spacex_status = SPACEX_MALLOC_FAILED;
    }
    ...
    for (size_t i = 0; i < SRD_RAND_DATA_NUM; i++)
    {
        ...
        spacex_status = seal_data_mrenclave(g_base_rand_buffer, SRD_RAND_DATA_LENGTH, &p_sealed_data, &sealed_data_size);
        if (SPACEX_SUCCESS != spacex_status)
        {
        ...
        ocall_save_file(&spacex_status, m_data_path.c_str(), reinterpret_cast<const uint8_t *>(p_sealed_data), SRD_RAND_DATA_LENGTH, STORE_TYPE_SRD);
        if (SPACEX_SUCCESS != spacex_status)
        {
            log_err("Save srd file(%s) failed! Error code:%lx\n", tmp_dir.c_str(), spacex_status);
            return spacex_status;
        }
    }
    ...
    spacex_status = save_m_hashs_file(tmp_dir.c_str(), m_hashs, SRD_RAND_DATA_NUM * HASH_LENGTH);
    if (SPACEX_SUCCESS != spacex_status)
    {
    ...
    ocall_rename_dir(&spacex_status, tmp_dir.c_str(), g_hash_path.c_str(), STORE_TYPE_SRD);
    if (SPACEX_SUCCESS != spacex_status)
    {
...
size_t srd_decrease(size_t change)
{
    ...
    {
        // Delete srd data
        ocall_delete_folder_or_file(&spacex_status, srd.c_str(), STORE_TYPE_SRD);
        if (SPACEX_SUCCESS != spacex_status)
        {
            log_warn("Delete path:%s failed! Error code:%lx\n", srd.c_str(), spacex_status);
        }
    }
...
spacex_status_t change_srd_task(long change, long *real_change)
{
    ...
    if (change > 0)
    {
        ...
        if (srd_num >= SRD_NUMBER_UPPER_LIMIT)
        {
            ...
            change = 0;
            spacex_status = SPACEX_SRD_NUMBER_EXCEED;
        }
...
spacex_status_t srd_get_file(const char *path, uint8_t **p_data, size_t *data_size)
{
    ...
    ocall_get_file(&spacex_status, path, p_data, data_size, STORE_TYPE_SRD);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
