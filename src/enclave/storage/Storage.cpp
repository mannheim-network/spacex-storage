...
spacex_status_t check_seal_file_dup(std::string cid);
 ...
 */
spacex_status_t storage_seal_file_start(const char *root, const char *root_b58)
{
    ...
    {
        return SPACEX_UPGRADE_IS_UPGRADING;
    }
    ...
    {
        return SPACEX_FILE_NUMBER_EXCEED;
    }
    // Check if file is duplicated
    if (SPACEX_SUCCESS != (spacex_status = check_seal_file_dup(root_cid.c_str())))
    {
 ...
 * @return: Seal result
 */
spacex_status_t storage_seal_file(const char *root,
                                 const uint8_t *data,
                                 size_t data_size,
                                 bool is_link,
                                 char *path,
                                 size_t /*path_size*/)
{
    ...
    if (p_plain_data == NULL || plain_data_sz == 0)
    {
        ...
        {
            return SPACEX_STORAGE_NEW_FILE_NOTFOUND;
        }
    ...
    spacex_status_t seal_ret = SPACEX_UNEXPECTED_ERROR;
    Defer defer([&seal_ret, &wl, &rcid, &root](){
        if (SPACEX_SUCCESS != seal_ret)
        {
    ...
    if (is_link)
    {
        ...
        for (long i = 0; i < links_json.size(); i++)
        {
            ...
            if (SPACEX_SUCCESS == (seal_ret = storage_get_file(s_path.c_str(), &p_sealed_data, &sealed_data_sz)))
            {
                break;
            }
        }
        ...
        if (SPACEX_SUCCESS != (seal_ret = unseal_data_mrsigner((sgx_sealed_data_t *)p_sealed_data, sealed_data_sz, &p_decrypted_data, &decrypted_data_sz)))
        {
            return seal_ret;
        }
 ...
 */
spacex_status_t storage_seal_file_end(const char *root)
{
    spacex_status_t spacex_status = SPACEX_UNEXPECTED_ERROR;
    std::string rcid(root);
    Workload *wl = Workload::get_instance();
    ...
    Defer defer([&spacex_status, &root, &wl](void) {
    ...
    for (auto m : file_json[FILE_BLOCKS].ObjectRange())
    {
        if (m.second.ToInt() != 0)
        {
            return SPACEX_STORAGE_INCOMPLETE_BLOCK;
        }
    }
    ...
    spacex_status = safe_ocall_get2(ocall_chain_get_block_info, block_info_buf, &info_buf_sz);
    if (SPACEX_SUCCESS == spacex_status)
    {
    ...
    spacex_status = persist_set_unsafe(cid_str, reinterpret_cast<const uint8_t *>(file_entry_json[FILE_HASH].ToBytes()),
 ...
 */
spacex_status_t storage_unseal_file(const char *path,
                                   ...
                                   size_t *p_decrypted_data_size)
{
    ...
    uint8_t *p_data = NULL;
    size_t data_size = 0;
    if (SPACEX_SUCCESS != (spacex_status = storage_get_file(path, &p_data, &data_size)))
    {
        return spacex_status;
    }
    ...
    // Do unseal
    if (SPACEX_SUCCESS != (spacex_status = unseal_data_mrsigner((sgx_sealed_data_t *)p_data, data_size, &p_unsealed_data, &unsealed_data_sz)))
    {
        return spacex_status;
    }
 ...
 */
spacex_status_t storage_delete_file(const char *cid)
{
    ...
    else
    {
        ...
        spacex_status = SPACEX_STORAGE_NEW_FILE_NOTFOUND;
    }
...
spacex_status_t check_seal_file_dup(std::string cid)
{
    ...
    if (wl->is_file_dup_nolock(cid, pos))
    {
        spacex_status = SPACEX_STORAGE_FILE_DUP;
        ...
        {
            spacex_status = SPACEX_STORAGE_FILE_SEALING;
        }
        else if (FILE_STATUS_DELETED == cur_s)
        {
            ...
            else
            {
                ...
                spacex_status = SPACEX_STORAGE_FILE_DELETING;
 ...
 * @return: Status
 */
spacex_status_t get_hashs_from_block(const uint8_t *block_data, size_t bs, std::vector<uint8_t *> &hashs)
{
 ...
 * @return: Get result
 */
spacex_status_t storage_get_file(const char *path, uint8_t **p_data, size_t *data_size)
{
    spacex_status_t spacex_status = SPACEX_SUCCESS;

    ocall_get_file(&spacex_status, path, p_data, data_size, STORE_TYPE_FILE);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
    ...
    {
        ocall_free_outer_buffer(&spacex_status, p_data);
        return SPACEX_MALLOC_FAILED;
    }
 ...
 * @return: Get status
 */
spacex_status_t storage_ipfs_get_block(const char *cid, uint8_t **p_data, size_t *data_size)
{
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    ocall_ipfs_get_block(&spacex_status, cid, p_data, data_size);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
 ...
 * @return: Cat status
 */
spacex_status_t storage_ipfs_cat(const char *cid, uint8_t **p_data, size_t *data_size)
{
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    ocall_ipfs_cat(&spacex_status, cid, p_data, data_size);
    if (SPACEX_SUCCESS != spacex_status)
    {
        return spacex_status;
    }
