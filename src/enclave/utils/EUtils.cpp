 ...
 * @return: Seal status
 */
spacex_status_t seal_data_mrenclave(const uint8_t *p_src, size_t src_len,
        sgx_sealed_data_t **p_sealed_data, size_t *sealed_data_size)
{
    ...
    *p_sealed_data = (sgx_sealed_data_t *)enc_malloc(sealed_data_sz);
    ...
    if (SGX_SUCCESS != sgx_status)
    {
        ...
        spacex_status = SPACEX_SEAL_DATA_FAILED;
        *p_sealed_data = NULL;
    }
 ...
 * @return: Seal status
 */
spacex_status_t seal_data_mrsigner(const uint8_t *p_src, size_t src_len,
        sgx_sealed_data_t **p_sealed_data, size_t *sealed_data_size)
{
    sgx_status_t sgx_status = SGX_SUCCESS;
    spacex_status_t spacex_status = SPACEX_SUCCESS;
    ...
    *p_sealed_data = (sgx_sealed_data_t *)enc_malloc(sealed_data_sz);
    ...
    if (SGX_SUCCESS != sgx_status)
    {
        ...
        free(*p_sealed_data);
        *p_sealed_data = NULL;
        return SPACEX_SEAL_DATA_FAILED;
    }
...
spacex_status_t validate_merkletree_json(json::JSON tree)
{
    ...
    for (int i = 0; i < tree[MT_LINKS].size(); i++)
    {
        ...
        {
            if (validate_merkletree_json(tree[MT_LINKS][i]) != SPACEX_SUCCESS)
            {
                spacex_status = SPACEX_INVALID_MERKLETREE;
                goto cleanup;
            }
        }
        std::string hash;
        ...
        if (tmp_hash == NULL)
        {
            spacex_status = SPACEX_UNEXPECTED_ERROR;
            goto cleanup;
        }
    ...
    {
        spacex_status = SPACEX_NOT_EQUAL;
        goto cleanup;
    }
 ...
 */
spacex_status_t unseal_data_mrsigner(const sgx_sealed_data_t *data,
                                    uint32_t data_size,
                                    ...
                                    uint32_t *decrypted_data_len)
{
    ...
    *p_decrypted_data = (uint8_t *)enc_malloc(*decrypted_data_len);
    ...
    if (SGX_SUCCESS != sgx_status)
    {
        ...
        return SPACEX_UNSEAL_DATA_FAILED;
    }
...
spacex_status_t safe_ocall_store2(ocall_store_type_t t, const uint8_t *u, size_t s)
{
    ...
    while (s > offset)
    {
        ...
        ret = ocall_safe_store2(&spacex_status,
                                t,
                                u + offset,
                                s,
                                partial_size,
...
spacex_status_t safe_ocall_get2(ocall_get2_f f, uint8_t *u, size_t *s)
{
    ...
    size_t os = *s;
    spacex_status_t c_ret = SPACEX_SUCCESS;
    ...
    if (SPACEX_SUCCESS != c_ret)
    {
        if (SPACEX_OCALL_NO_ENOUGH_BUFFER == c_ret)
        {
            free (u);
            if (os == *s)
            {
