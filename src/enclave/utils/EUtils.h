#ifndef _SPACEX_E_UTILS_H_
#define _SPACEX_E_UTILS_H_
...
spacex_status_t seal_data_mrenclave(const uint8_t *p_src, size_t src_len, sgx_sealed_data_t **p_sealed_data, size_t *sealed_data_size);
spacex_status_t seal_data_mrsigner(const uint8_t *p_src, size_t src_len, sgx_sealed_data_t **p_sealed_data, size_t *sealed_data_size);
spacex_status_t unseal_data_mrsigner(const sgx_sealed_data_t *data, uint32_t data_size, uint8_t **p_decrypted_data, uint32_t *decrypted_data_len);

spacex_status_t validate_merkletree_json(json::JSON tree);
...
spacex_status_t safe_ocall_store2(ocall_store_type_t t, const uint8_t *u, size_t s);
spacex_status_t safe_ocall_get2(ocall_get2_f f, uint8_t *u, size_t *s);
 ...
 */
spacex_status_t vector_end_insert(std::vector<T> &v, const T *data, size_t data_size)
{
    try
    {
 ...
 * @return: Insert result
 */
spacex_status_t vector_end_insert(std::vector<T> &v, std::vector<T> &s)
{
    try
    {
 ...
 * @return: Insert result
 */
spacex_status_t vector_end_insert(std::vector<T> &v, std::string str)
{
    try
    {
