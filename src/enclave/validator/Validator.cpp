...
void validate_srd()
{
    ...
    for (auto hash : g_del_srd_v)
    {
        ...
        ocall_delete_folder_or_file(&spacex_status, del_path.c_str(), STORE_TYPE_SRD);
    }
...
void validate_srd_real()
{
    ...
    spacex_status_t spacex_status = srd_get_file(get_m_hashs_file_path(srd_hex.c_str()).c_str(), &m_hashs_org, &m_hashs_size);
    ...
    spacex_status = srd_get_file(leaf_path.c_str(), &leaf_data, &leaf_data_len);
    if (SPACEX_SUCCESS != spacex_status)
    {
        if (!wl->add_srd_to_deleted_buffer(srd_index))
        {
        ...
        log_err("Get srd(%s) block(%s) failed. Error code:%x\n", 
                g_hash_hex.c_str(), hexstring_safe(m_hashs + srd_block_index * HASH_LENGTH, HASH_LENGTH).c_str(), spacex_status);
        deleted = true;
        return;
    }
...
void validate_meaningful_file_real()
{
    ...
    uint8_t *p_tree = NULL;
    size_t tree_sz = 0;
    spacex_status_t spacex_status = persist_get_unsafe(root_cid, &p_tree, &tree_sz);
    if (SPACEX_SUCCESS != spacex_status)
    {
    ...
    for (auto check_block_idx : block_idx_s)
    {
        ...
        spacex_status = storage_get_file(leaf_path.c_str(), &p_sealed_data, &sealed_data_sz);
        if (SPACEX_SUCCESS != spacex_status)
        {
            if (wl->is_in_deleted_file_buffer(root_cid))
            {
