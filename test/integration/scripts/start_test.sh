...
### Start spacex-storage
cd $testdir
rm -rf sworker_base_path
verbose INFO "starting spacex-storage..." h
./bin/spacex-storage -c etc/Config.json --offline --debug &>$sworkerlog &
sworkerpid=$!
sleep 10
while true; do
    ...
    if [ $? -eq 0 ] ; then
        ...
        verbose ERROR "start spacex sworker failed! Please check $errfile for details."
        kill -9 $sworkerpid
        exit 1
    fi
    sleep 3
