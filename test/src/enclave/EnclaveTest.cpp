...
spacex_status_t ecall_srd_increase_test(const char *uuid)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
...
spacex_status_t ecall_get_file_info(const char *data)
{
    Workload *wl = Workload::get_instance();
    sgx_thread_mutex_lock(&wl->file_mutex);
...
spacex_status_t ecall_gen_upgrade_data_test(size_t block_height)
{
    return id_gen_upgrade_data_test(block_height);
}

spacex_status_t ecall_gen_and_upload_work_report_test(const char *block_hash, size_t block_height)
{
    if (ENC_UPGRADE_STATUS_NONE != Workload::get_instance()->get_upgrade_status())
    {
    ...
    spacex_status_t ret = gen_and_upload_work_report_test(block_hash, block_height, 0, false);
